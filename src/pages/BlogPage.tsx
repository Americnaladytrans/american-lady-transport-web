...
const inferPostType = (title: string): string => {
  const lower = title.toLowerCase();
  if (lower.includes("roundup") || lower.includes("industry news")) {
...
const BlogPage = () => {
  ...
  const posts: BlogPost[] = useMemo(() => {
    return [...(blogPostsData as Omit<BlogPost, "post_type">[])]
      .sort((a, b) => new Date(b.published_at).getTime() - new Date(a.published_at).getTime())
      .map((p) => ({ ...p, post_type: inferPostType(p.title) }));
  }, []);
